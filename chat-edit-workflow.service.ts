import { Component, Input, Output, EventEmitter, ChangeDetectorRef, OnChanges, OnInit, SimpleChanges } from '@angular/core';
...
export class ParagraphEditsConsolidatedComponent implements OnChanges, OnInit {
  ...
  ngOnInit(): void {
    // Initialize highlights on first load
    this.initializeHighlights();
  }

  get allParagraphsDecided(): boolean {
    // Check if all paragraph edits are decided
    const editsDecided = allParagraphsDecided(this.paragraphEdits);
  ...
  get allParagraphFeedbackDecided(): boolean {
    ...
    return this.paragraphEdits.every(para => {
      ...
      // Check if all editorial feedback items are decided
      const feedbackTypes = Object.keys((para as any).editorial_feedback || {});
      for (const editorType of feedbackTypes) {
        const feedbacks = ((para as any).editorial_feedback as any)[editorType] || [];
  ...
  private initializeHighlights(): void {
    ...
    this.paragraphEdits.forEach((p: any, idx: number) => {
      ...
      const highlighted = this.highlightAllFeedbacks(p as any);
      // Always initialize display fields with highlights if not already set
      // This ensures yellow highlights show by default for unreviewed feedback
      if (!p.displayOriginal) {
        p.displayOriginal = highlighted.original;
      }
      if (!p.displayEdited) {
        p.displayEdited = highlighted.edited;
      }
    });
  ...
  private updateParagraphApprovedStatus(para: any): void {
  ...
  applyEditorialFix(para: any, editorType: string, fb: any): void {
    ...
    // Update paragraph's approved status if all feedback items are now decided
  ...
  highlightAllFeedbacks(para: ParagraphEdit | ParagraphFeedback | null | undefined): { original: string, edited: string } {
    ...
    const originalItems: Array<{text: string, approved: boolean | null, start: number, end: number, feedbackId: string}> = [];
    const editedItems: Array<{text: string, approved: boolean | null, start: number, end: number, feedbackId: string}> = [];

    const editorial = (para as any)?.editorial_feedback || {};
    const paraIndex = (para as any)?.index ?? 0;

    // Collect all issues from original text
    Object.keys(editorial).forEach(editorType => {
      const feedbacks = (editorial as any)[editorType] || [];
      feedbacks.forEach((fb: any, fbIndex: number) => {
        // Create unique ID for this feedback item
        const feedbackId = `fb-${paraIndex}-${editorType}-${fbIndex}`;
    ...
    originalItems.forEach(item => {
      ...
      if (item.approved === true) {
        ...
        highlighted = `<span class="strikeout highlight-yellow feedback-highlight" data-feedback-id="${item.feedbackId}">${item.text}</span>`;
      } else if (item.approved === false) {
        // Rejected: green (opposite of approve)
        highlighted = `<span class="highlight-green feedback-highlight" data-feedback-id="${item.feedbackId}">${item.text}</span>`;
      } else {
        // Unreviewed: yellow
        highlighted = `<span class="highlight-yellow feedback-highlight" data-feedback-id="${item.feedbackId}">${item.text}</span>`;
      }
  ...
  approveAllFeedback(): void {
    ...
    this.paragraphEdits.forEach((para: any) => {
      ...
      // Update paragraph's approved status
      this.updateParagraphApprovedStatus(para);
      // Clear display properties so highlightAllFeedbacks() handles all highlighting
      para.displayOriginal = undefined;
      para.displayEdited = undefined;
